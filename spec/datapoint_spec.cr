...
describe "knx datapoint helper" do
  describe KNX::TwoByteFloatingPoint do
    it "should parse encoded floating point numbers" do
      dp = KNX::TwoByteFloatingPoint.new(Bytes[0x8A, 0x24])
      dp.value.should eq(-30)
      dp.to_bytes.should eq(Bytes[0x8A, 0x24])

      dp = KNX::TwoByteFloatingPoint.new(Bytes[0x0C, 0x7E])
      dp.value.should eq(23.0)
      dp.to_bytes.should eq(Bytes[0x0C, 0x7E])

      dp = KNX::TwoByteFloatingPoint.new(19.5)
      dp.value.should eq(19.5)
      dp.to_bytes.should eq(Bytes[0x07, 0x9E])

      dp = KNX::TwoByteFloatingPoint.new(Bytes[0x02, 0x44])
      dp.value.should eq(5.8)
      dp.to_bytes.should eq(Bytes[0x02, 0x44])

      dp = KNX::TwoByteFloatingPoint.new(Bytes[0x85, 0x76])
      dp.value.should eq(-6.5)
      dp.to_bytes.should eq(Bytes[0x85, 0x76])

      dp = KNX::TwoByteFloatingPoint.new(36.7)
      dp.value.should eq(36.7)
      dp.to_bytes.should eq(Bytes[0x0F, 0x2B])

      dp = KNX::TwoByteFloatingPoint.new(Bytes[0x00, 0x00])
      dp.value.should eq(0)
      dp.to_bytes.should eq(Bytes[0x00, 0x00])
    end

    it "should parse 2 byte floating points" do
      dt = KNX::TwoByteFloatingPoint.new(Bytes[0x06, 0xa0])
      dt.value.should eq(16.96)
      dt.to_slice.should eq(Bytes[0x06, 0xa0])

      dt = KNX::TwoByteFloatingPoint.new(Bytes[0xF8, 0x01])
      dt.value.should eq(-670760.96)
      dt.to_slice.should eq(Bytes[0xF8, 0x01])
    end
  end
  ...
  describe KNX::DateTime do
    it "should parse encoded date times" do
      ...
      dt.to_slice.should eq(Bytes[0x75, 0x0B, 0x1C, 0x17, 0x07, 0x18, 0x04, 0x80])
    end
  end
  ...
  describe KNX::DpTime do
    it "should parse encoded times" do
      ...
      dt.to_slice.should eq(Bytes[0x4D, 0x17, 0x2A])
    end
  end
  ...
  describe KNX::Date do
    it "should parse an early date" do
      ...
      dt.to_slice.should eq(Bytes[0x1F, 0x01, 0x5A])
    end
    ...
    it "should parse later date" do
      ...
      dt.to_slice.should eq(Bytes[0x04, 0x01, 0x02])
    end
  end
  ...
  describe KNX::DpString do
    it "should parse strings" do
      ...
      dt.to_slice.should eq(Bytes[0x4B, 0x4E, 0x58, 0x20, 0x69, 0x73, 0x20, 0x4F, 0x4B, 0x00])
    end
    ...
    it "should parse long strings" do
      ...
      dt.to_bytes.should eq(bytes)
      dt.value = "AAAAABBBBBCCCCDD"
      dt.to_slice.should eq(bytes)
    end
  end
  ...
  describe KNX::FourByteFloat do
    it "should parse a 32bit float" do
      ...
      dt.to_slice.should eq(Bytes[0x42, 0xEF, 0x00, 0x00])
    end
    ...
    it "should parse a different 32bit float" do
      ...
      dt.to_bytes.should eq(Bytes[0x3F, 0x71, 0xEB, 0x86])
      dt.value = 0.945000052452.to_f32
      dt.to_slice.should eq(Bytes[0x3F, 0x71, 0xEB, 0x86])
    end
  end
end
